...
- Ensure the MR is conflict-free with main.
- Keep GitLab CI green and fix failures when they occur.
- Squash-merge the MR once pipelines pass.
- Do not yield to the user until the MR is merged; keep the watcher loop running
  unless blocked.
...
- `glab` CLI is authenticated.
- You are on the MR branch with a clean working tree.

## Steps

1. Locate the MR for the current branch.
...
7. Watch GitLab pipelines until complete.
8. If pipelines fail, pull logs, fix the issue, commit with the `commit` skill,
   push with the `push` skill, and re-run checks.
9. When all checks are green and review feedback is addressed, squash-merge and
   delete the branch using the MR title/description for the squash message.
...
12. **Ambiguity gate:** When ambiguity blocks progress, use the clarification
    flow (assign MR to the current GitLab user, mention them, wait for
    response). Do not implement until ambiguity is resolved.
...
13. **Per-comment mode:** For each review comment, choose one of: accept,
    clarify, or push back. Reply inline (or in the MR thread for Codex
    reviews) stating the mode before changing code.
14. **Reply before change:** Always respond with intended action before pushing
    code changes (inline for review comments, MR thread for Codex reviews).
...
mr_description=$(printf '%s' "$mr_json" | jq -r '.description // empty')

# Check mergeability and conflicts
merge_status=$(glab api "projects/:fullpath/merge_requests/$mr_iid" \
  | jq -r '.detailed_merge_status // .merge_status')

if [ "$merge_status" = "conflicts" ] || [ "$merge_status" = "cannot_be_merged" ]; then
...
# Wait for review feedback: Codex reviews arrive as MR notes that start
# with "## Codex Review — <persona>". Treat them like reviewer feedback: reply
# with a `[codex]` MR note acknowledging the findings and whether you're
# addressing or deferring them.
while true; do
  glab api "projects/:fullpath/merge_requests/$mr_iid/notes" \
    | jq -r '.[] | select(.body | startswith("## Codex Review")) | .id' | rg -q '.' \
    && break
  sleep 10
done

# Watch GitLab CI
if ! glab ci status --branch "$branch" --live; then
  glab ci status --branch "$branch"
  # Identify failing job and inspect logs:
...
- 3: CI checks failed
- 4: MR head updated (autofix commit detected)

## Failure Handling

- If pipelines fail, pull details with `glab ci status --output json` and
  `glab ci trace <job-id>`, then fix locally, commit with the `commit` skill,
  push with the `push` skill, and re-run the watch.
...
- Codex review jobs retry on failure and are non-blocking; use the presence of
  `## Codex Review — <persona>` MR notes (not job status) as the signal
  that review feedback is available.
- Do not enable auto-merge; this repo has no required checks so auto-merge can
  skip tests.
- If the remote MR branch advanced due to your own prior force-push or merge,
  avoid redundant merges; re-run the formatter locally if needed and
  `git push --force-with-lease`.

## Review Handling

- Codex reviews now arrive as MR notes posted by GitLab CI. They
...
- Fetch review discussions via `glab api` and reply with a prefixed comment.
- Use merge request discussion endpoints to find inline feedback:
  - List MR discussions:
    ```
    glab api "projects/:fullpath/merge_requests/<mr_iid>/discussions"
    ```
  - MR notes (top-level discussion):
    ```
    glab api "projects/:fullpath/merge_requests/<mr_iid>/notes"
    ```
  - Reply to a specific discussion:
    ...
    glab api --method POST \
      ...
      --field body='[codex] <response>'
    ```
- To add a root-level MR note:
  ```
  glab mr note <mr_iid> --message '[codex] <response>'
  ```
- A 404 on reply typically means the wrong endpoint, wrong MR IID, wrong
  discussion ID, or insufficient scope; verify by listing discussions first.
- All GitLab comments generated by this agent must be prefixed with `[codex]`.
- For Codex review MR notes, reply in the MR thread (not a review
...
- If feedback requires changes:
  - For inline review comments (human), reply with intended fixes
    (`[codex] ...`) **as a reply in the original discussion** using the
    discussion notes endpoint (do not use root-level MR notes for inline
    discussions).
  - Implement fixes, commit, push.
  - Reply with the fix details and commit sha (`[codex] ...`) in the same place
    you acknowledged the feedback (root-level MR note for Codex reviews,
    discussion reply for inline review comments).
  - The land watcher treats Codex review MR notes as unresolved until a newer
    `[codex]` MR note is posted acknowledging the findings.
- Only request a new Codex review when you need a rerun (e.g., after new
  ...
  - Before requesting a new Codex review, re-run the land watcher and ensure
    ...
    replies).
  - After pushing new commits, the Codex review pipeline will rerun on MR
